...
type Client struct {
	...
	before         []gotransport.RequestFunc
	after          []gotransport.ClientResponseFunc
	finalizer      gotransport.ClientFinalizerFunc
...
func SetClient(client gotransport.HTTPClient) ClientOption {
	return func(c *Client) { c.client = client }
}

// ClientBefore sets the RequestFuncs that are applied to the outgoing HTTP
// request before it's invoked.
func ClientBefore(before ...gotransport.RequestFunc) ClientOption {
...
func ClientAfter(after ...gotransport.ClientResponseFunc) ClientOption {
	return func(c *Client) { c.after = append(c.after, after...) }
}

// ClientFinalizer is executed at the end of every HTTP request.
// By default, no finalizer is registered.
func ClientFinalizer(f gotransport.ClientFinalizerFunc) ClientOption {
...
func (c Client) Endpoint() endpoint.Endpoint {
	return func(ctx context.Context, request interface{}) (interface{}, error) {
		...
		if c.finalizer != nil {
			defer func() {
				if resp != nil {
					ctx = context.WithValue(ctx, gotransport.ContextKeyResponseHeaders, resp.Header)
					ctx = context.WithValue(ctx, gotransport.ContextKeyResponseSize, resp.ContentLength)
				}
				c.finalizer(ctx, err)
			}()
		}

...
type Server struct {
	...
	after        []gotransport.ServerResponseFunc
	errorEncoder gotransport.ErrorEncoder
	finalizer    gotransport.ServerFinalizerFunc
	logger       log.Logger
}
...
// request is decoded.
func ServerBefore(before ...gotransport.RequestFunc) ServerOption {
...
func ServerAfter(after ...gotransport.ServerResponseFunc) ServerOption {
...
func ServerErrorEncoder(ee gotransport.ErrorEncoder) ServerOption {
...
func ServerFinalizer(f gotransport.ServerFinalizerFunc) ServerOption {
...
func DefaultErrorEncoder(ctx context.Context, err error, w http.ResponseWriter) {
	w.Header().Set("Content-Type", ContentType)
	if headerer, ok := err.(gotransport.Headerer); ok {

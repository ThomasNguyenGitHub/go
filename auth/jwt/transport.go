...
import (
	...
	"github.com/ThomasNguyenGitHub/go/transport/grpc"
	"github.com/ThomasNguyenGitHub/go/transport/http"
)
...
func HTTPToContext() http.RequestFunc {
	return func(ctx context.Context, r *stdhttp.Request) context.Context {
		...
		return context.WithValue(ctx, local.Getenv("JWT_TOKEN_CONTENT_KEY"), token)
	}
}
...
func GRPCToContext() grpc.ServerRequestFunc {
	return func(ctx context.Context, md metadata.MD) context.Context {
		...
		if ok {
			ctx = context.WithValue(ctx, local.Getenv("JWT_TOKEN_CONTENT_KEY"), token)
		}
